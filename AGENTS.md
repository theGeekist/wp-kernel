...
- Non-negotiables:
    ...
    - Lifecycle phases, namespaces, and CLI exit codes come from `@wpkernel/core/contracts`-never inline the wpk namespace or exit code numbers.
...
- Current release train: **v0.12.0 (pre-1.0)**. All publishable packages share the same semantic version sourced from the root `package.json`.
