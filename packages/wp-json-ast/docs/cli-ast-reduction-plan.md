...
Phase 2.6 laid the groundwork by centralising resource-specific query, mutation, and cache helpers inside `@wpkernel/wp-json-ast`. Every bundle in Task 3.2 should compose those accessors instead of rebuilding AST fragments, ensuring the CLI depends on declarative descriptors only. For example, the CLI now stitches together the WP_Post list/meta/taxonomy query helpers from Subtask 2.6.b, the mutation macros from Subtask 2.6.c, and the mutation route primitives from Subtask 2.6.d alongside the shared route-set fan-out delivered in Subtask 3.2.a.
...
- `packages/cli/src/next/builders/php/resource/wpPost/**` (route fan-out plus mutation macros) → compose the `@wpkernel/wp-json-ast` query, mutation, and helper primitives, wrapped by `createPhpWpPostRoutesHelper` feeding `createPhpResourceControllerHelper`.
...
- **Scope:** Provide a pipeline helper that translates WP_Post IR into the configuration expected by `buildWpPostRouteBundle`, returning the bundle to the resource controller helper.
...
This document anchors Phase 1 and tracks Phase 2-3 migrations. Future contributors should append findings, inventories, and decisions here so the roadmap evolves alongside the codebase while keeping the CLI stable throughout the migration.
