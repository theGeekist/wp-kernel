...
- **PHP AST parity** – The next pipeline now emits all PHP artifacts through AST helpers (resource controllers, policy helper, persistence registry, index file) while the legacy printers remain only for the legacy CLI. Keep `packages/cli/docs/php-json-ast-migration.md` updated as we retire the remaining legacy-only printers.
