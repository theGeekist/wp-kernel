...
- **Current state:** `NextApplyCommand` loads the kernel config, executes `createPatcher`, and prints the manifest summary. It does not enforce git hygiene, create backups, honour `--yes/--backup/--force`, or append to `.wpk-apply.log`.
