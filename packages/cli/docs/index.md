...
- **Version guard** – The CLI operates on the unified **v0.4.x (pre-1.0)** line. Reserved version slots live in [MVP Plan](./mvp-plan.md); claim and update them as tasks ship so parallel agents never collide.
...
- **[PHP AST Migration Tasks](./php-ast-migration-tasks.md)** – status tracker for AST parity (what ships vs. remaining tasks: wp-option, transient, block printers, string-printer retirement).
