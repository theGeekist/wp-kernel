...
| 0.10.3                                                                                              | Task 39 – Init adoption guardrails                  | ✓ shipped  | `wpk init` now partitions kernel vs author templates, skips author-owned collisions, logs detected plugin markers (composer autoload metadata and root-level plugin headers), and ships regression coverage for clean and established plugin workspaces. |
