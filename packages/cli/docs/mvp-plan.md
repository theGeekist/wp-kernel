...
| 0.4.3 | Task 3 – End-to-end generate coverage             | ✓ shipped (this PR) | Integration test snapshots cover PHP + AST artefacts.            |
| 0.4.4 | Task 4 – Driver configuration & documentation     | ✓ shipped (this PR) | Update docs + exports alongside code.                            |
...
| 3   | End-to-end generate coverage             | Execute the PHP builder + writer against a workspace fixture and assert emitted `.php`/`.ast.json` files without touching string-based printers.                                                 | 0.4.3 (patch)                               | Baseline + `pnpm --filter @wpkernel/cli test -- --runTestsByPath packages/cli/src/next/builders/php/__tests__/generate.integration.test.ts` |
...
Task 4 is now ✓ shipped (this PR). `createPhpBuilder` exposes driver overrides, `createPhpProgramWriterHelper` forwards them to `@wpkernel/php-driver`, and the docs outline how to point the pretty-printer at custom binaries or relocated bundles.
