...
| 0.4.2 | Task 2 – Pretty printer ESM path fix           | ⬜ available | Requires PHP driver tests.                                       |
| 0.4.3 | Task 3 – End-to-end generate coverage          | ⬜ available | Use next workspace fixtures only.                                |
| 0.4.4 | Task 4 – Driver configuration plumbing         | ⬜ available | Update docs + exports alongside code.                            |
...
| 2   | Pretty printer ESM path fix       | Extend `buildPhpPrettyPrinter` to accept an `importMetaUrl` option, update the ESM entry point, and add tests covering CJS + ESM resolution.                                                     | 0.4.2 (patch)                               | Baseline + `pnpm --filter @wpkernel/php-driver test`                         |
...
| 4   | Driver configuration plumbing     | Thread optional PHP driver settings (binary, script path, import meta URL) through `createPhpBuilder`/`createPhpProgramWriterHelper`, update exports, and document the knobs.                    | 0.4.4 (patch)                               | Baseline + `pnpm --filter @wpkernel/php-driver test`                         |
