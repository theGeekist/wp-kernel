...
- Re-runs generate on change (no automatic `apply`).
- Optionally auto-applies JS artefacts when `--auto-apply-php` is provided; otherwise require explicit `wpk apply`.
