...
const COMMAND_HELP: CommandHelpMap = {
	...
	generate: {
		...
		details: [
			...
			'  • `--allow-dirty` (`-D`) - run even if git has pending changes (useful in CI sandbox pipelines).',
			'',
			'Outputs:',
...
const COMMAND_ORDER: CommandHelpName[] = [
	'create',
	'init',
	'generate',
	'apply',
...
const commandSummaryLines = COMMAND_ORDER.map((name) => {
	const { summary } = COMMAND_HELP[name];
	return `  • wpk ${name}  ${summary}`;
});
