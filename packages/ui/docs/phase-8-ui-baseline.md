...
| ------ | ------------------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0.11.1 | Task 46 – DataViews schema expansion  | ⬜ Planned | Extend resource metadata ingestion so `attachUIBindings()` recognises saved layouts, server-driven columns, and menu metadata before a controller instantiates. | [`attachUIBindings`](../src/runtime/attachUIBindings.ts), [`ResourceDataViewConfig`](../src/dataviews/types.ts)                                   |
| 0.11.2 | Task 47 – Async boundaries & notices  | ⬜ Planned | Introduce shared empty/error/permission boundaries and toast/reporting helpers wired to controller reporters and `useListResult()`.                             | [`ResourceDataView`](../src/dataviews/ResourceDataView.tsx), [`useDataViewActions`](../src/dataviews/resource-data-view/use-data-view-actions.ts) |
| 0.11.3 | Task 48 – Interactivity bridge        | ⬜ Planned | Publish a `createDataViewInteraction()` helper that syncs DataView controllers with `defineInteraction`, forwarding selections, filters, and cache hydration.   | [`attachUIBindings`](../src/runtime/attachUIBindings.ts), [`defineInteraction`](../../core/src/interactivity/defineInteraction.ts)                |
...
| 0.11.5 | Task 50 – CLI DataView alignment      | ⬜ Planned | Update CLI generators and manifests to emit the richer DataViews config, enqueue UI assets, and snapshot the auto-registration metadata.                        | [`generate.ts`](../../cli/src/commands/generate.ts), [`workspace/filesystem.ts`](../../cli/src/next/workspace/filesystem.ts)                      |
...
### Patch 0.11.1 - Task 46: DataViews schema expansion (⬜ Planned)
...
### Patch 0.11.2 - Task 47: Async boundaries & notices (⬜ Planned)

Introduce shared boundaries for loading, empty, error, and permission-denied states so every `ResourceDataView` renders consistent feedback without bespoke components. Wire the boundaries to the controller reporter and `useListResult()` so async fetch paths surface errors alongside DataViews' own inline UI.【F:packages/ui/src/dataviews/resource-data-view/use-list-result.ts†L1-L27】 Extend `useDataViewActions()` to trigger standard toast helpers on success/failure, reusing capability decisions and invalidate hooks that already flow through the controller when invoking actions.【F:packages/ui/src/dataviews/ResourceDataView.tsx†L24-L115】【F:packages/ui/src/dataviews/resource-data-view/use-data-view-actions.ts†L1-L200】 Update tests under `packages/ui/src/dataviews/__tests__/` to snapshot the new boundaries.

### Patch 0.11.3 - Task 48: Interactivity bridge (⬜ Planned)
